...
#define Q_GRAM_STRAND_SIZE 200 //org 200 for minilab data
...
inline void q_gram_sequence(int idx){
  ...
  // cout << "--- " << "q_gram dict element " << temp_gram_string << endl;
}
...
int hamming_distance(int i , int j){
  // cout << "qgram value " << q_gram_dict[i] << endl;
...
void local_clustering(vector<vector<int> > &clusters){
  ...
  for(int k=0;k<num_local_iter;k++){
    ...
    size += hash_values.size();
    // cout << "hash values size:" << size << endl;
    // cout << "hash values :" << hash_values[0].rep<< endl;
...
int main(){
  ...
  cout << initial_clusters.size() << endl; 
  ...
  cout << q_gram_dict.size() << endl; //input size 
  create_q_gram_dict(); //creates qgram thread -> qgram sequence -> generate qgram dict -> binary strings
  ...
  ofstream myfile ("./bin_sig/error3/bin_synth_ref_strand.txt");
  if (myfile.is_open())
  {
    for(int count = 0; count < q_gram_dict.size(); count ++){
        myfile << q_gram_dict[count] << endl ;
    }
    myfile.close();
  }
